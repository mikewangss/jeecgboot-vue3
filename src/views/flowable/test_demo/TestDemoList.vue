<template>
  <a-card :bordered="false">
          ...
          <template v-if="toggleSearchStatus">
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="生日">
                <j-date placeholder="请选择生日" v-model="queryParam.birthday" />
              </a-form-item>
            </a-col>
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
              <a-form-item label="用户编码">
                <a-input placeholder="请输入用户编码" v-model="queryParam.userCode" />
              </a-form-item>
            </a-col>
            <a-col :xl="6" :lg="7" :md="8" :sm="24">
    ...
    <div>
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <act-apply-btn @success="loadData" :data-id="record.id" :variables="{ assigneeList: [] }" />
          {{ isCanApply(record) }}
          <act-handle-btn @success="loadData" :data-id="record.id" :type="3" text="重新提交" />
          {{ isCanReApply(record) }}
          <act-handle-btn
            @success="loadData"
            :data-id="record.id"
            ...
            :candidate-users="[
              ...
              { username: 'jeecg', realname: 'jeecg账号' },
            ]"
          />
          {{ isCanPass(record) }}
          <act-handle-btn @success="loadData" :data-id="record.id" :type="1" text="驳回" />
          {{ isCanBacke(record) }}
          <act-handle-btn
            @success="loadData"
            :data-id="record.id"
          ...
          />
          {{ isCanBacke(record) }}
          <act-cancel-btn @success="loadData" :data-id="record.id" />
          {{ isCanRecall(record) }}
    ...
    <test-demo-modal ref="modalForm" @ok="modalFormOk" />
  </a-card>
</template>
