<template>
  <div class="search">
    <a-card>
      ...
      <div :style="{ height: svgHeight }" v-if="true">
        <bpmnModeler class="svg" ref="bpm" :xml="xmlData" is-view="true" />
      </div>
    </a-card>
  ...
  export default {
    ...
    methods: {
      ...
      async getModelDetail() {
        // 发送请求，获取xml
        await readXmlByDataId(this.dataId).then((res) => {
      ...
      // 流程进行情况
      async getFlowViewer() {
        await getFlowViewerByDataId(this.dataId).then((res) => {
